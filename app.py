...
def api_state() -> JSONResponse:
    return JSONResponse(dashboard_payload(), headers=NO_CACHE_HEADERS)
...
async def api_create_event(event: EventIn) -> JSONResponse:
    ...
    if event.activity not in allowed:
        return JSONResponse({"error": "Unknown activity"}, status_code=400, headers=NO_CACHE_HEADERS)
...
async def api_update_event(event_id: int, event: EventUpdate) -> JSONResponse:
    ...
    if not event_time:
        return JSONResponse({"error": "Invalid time"}, status_code=400, headers=NO_CACHE_HEADERS)
    is_accident = normalize_is_accident(event.activity, event.is_accident)
    with get_db() as conn:
        exists = conn.execute("SELECT id FROM events WHERE id = ?", (event_id,)).fetchone()
        if not exists:
            return JSONResponse({"error": "Event not found"}, status_code=404, headers=NO_CACHE_HEADERS)
        conn.execute(
            """
            UPDATE events
...
async def api_settings(settings: SettingsIn) -> JSONResponse:
    ...
    if birth and not parse_date(birth):
        return JSONResponse({"error": "Invalid birth date"}, status_code=400, headers=NO_CACHE_HEADERS)
...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    .boot-status{margin-bottom:14px;padding:12px 14px;border-radius:14px;background:var(--card-strong);border:1px solid var(--border);color:var(--muted)}
    .boot-status.hidden{display:none}
    .boot-status.error{background:#40202b;border-color:#fb7185;color:#fecdd3}
...
<body>
  <div class="wrap">
    ...
    <div id="boot-status" class="boot-status">Loading latest puppy data...</div>
  ...
  <script>
    ...
    function setBootStatus(message, isError = false) {
      if (!message) {
        ...
        els.bootStatus.classList.remove('error');
        return;
      }
      els.bootStatus.textContent = message;
      els.bootStatus.classList.remove('hidden');
      els.bootStatus.classList.toggle('error', isError);
    }

    async function loadState() {
      setBootStatus('Loading latest puppy data...');
    ...
    function connectWs() {
      ...
      ws.onerror = () => {
        els.wsStatus.textContent = 'Connection issue';
      };
      ws.onclose = () => {
    ...
    async function boot() {
      try {
        ...
        console.error(error);
        setBootStatus('Could not load the latest data. Pull down to refresh or fully reopen the app.', true);
